...
def display_enrollment_details(enrollment):
    ...
    with col2:
        if st.button("✏️ Edit Enrollment", key=f"edit_enrollment_{enrollment.get('id', '')}"):
            st.session_state.selected_enrollment_for_edit = enrollment
            st.rerun()

        if st.button("🗑️ Delete Enrollment", key=f"delete_enrollment_{enrollment.get('id', '')}"):
            st.session_state.delete_enrollment = enrollment
            st.rerun()

    # Show edit form if editing
    if hasattr(st.session_state, "selected_enrollment_for_edit") and st.session_state.selected_enrollment_for_edit.get(
        "id"
    ) == enrollment.get("id"):
        edit_enrollment_form(st.session_state.selected_enrollment_for_edit)

    # Show delete confirmation if deleting
    if hasattr(st.session_state, "delete_enrollment") and st.session_state.delete_enrollment.get(
        "id"
    ) == enrollment.get("id"):
...
def display_enrollments():
    ...
    if result["success"]:
        ...
        with col2:
            progress_filter = st.selectbox("Filter by progress", ["All", "0-25%", "26-50%", "51-75%", "76-100%"])
        with col3:
        ...
        if status_filter != "All":
            filtered_enrollments = [e for e in filtered_enrollments if e.get("status") == status_filter]

        if progress_filter != "All":
            if progress_filter == "0-25%":
                filtered_enrollments = [e for e in filtered_enrollments if 0 <= e.get("progress", 0) <= 25]
            elif progress_filter == "26-50%":
                filtered_enrollments = [e for e in filtered_enrollments if 26 <= e.get("progress", 0) <= 50]
            elif progress_filter == "51-75%":
                filtered_enrollments = [e for e in filtered_enrollments if 51 <= e.get("progress", 0) <= 75]
            elif progress_filter == "76-100%":
                filtered_enrollments = [e for e in filtered_enrollments if 76 <= e.get("progress", 0) <= 100]
        ...
        if sort_by == "Status":
            ...
            filtered_enrollments.sort(key=lambda x: x.get("enrolled_at", ""), reverse=True)

        # Display filtered results
        if filtered_enrollments:
            st.info(f"Showing {len(filtered_enrollments)} of {len(enrollments)} enrollments")
            
            # Pagination
            items_per_page = 5
            total_pages = (len(filtered_enrollments) + items_per_page - 1) // items_per_page
            ...
            if total_pages > 1:
                ...
                with nav_col1:
                    if st.button("⬅️ Prev", disabled=st.session_state.enrollments_current_page <= 1, key="enrollments_prev"):
                        st.session_state.enrollments_current_page -= 1
                        st.rerun()
                ...
                with nav_col2:
                    page = st.selectbox(
                        ...
                        on_change=lambda: setattr(st.session_state, "enrollments_current_page", st.session_state.enrollments_page_selector)
                    )
                
                with nav_col3:
                    if st.button("Next ➡️", disabled=st.session_state.enrollments_current_page >= total_pages, key="enrollments_next"):
                        st.session_state.enrollments_current_page += 1
                        st.rerun()
                
                # Calculate start and end indices
                start_idx = (st.session_state.enrollments_current_page - 1) * items_per_page
                end_idx = min(start_idx + items_per_page, len(filtered_enrollments))
                
                st.info(f"Showing enrollments {start_idx + 1}-{end_idx} of {len(filtered_enrollments)}")
