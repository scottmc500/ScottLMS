...
from frontend.components.forms import (delete_course_confirmation,
                                       delete_user_confirmation,
                                       edit_course_form, edit_user_form)
...
def display_user_details(user):
    ...
    if hasattr(st.session_state, "edit_user") and st.session_state.edit_user.get(
        "id"
    ) == user.get("id"):
        edit_user_form(user)
...
def display_course_details(course):
    ...
    # Show edit form if editing
    if hasattr(st.session_state, "edit_course") and st.session_state.edit_course.get(
        "id"
    ) == course.get("id"):
...
def display_users():
    ...
    if result["success"]:
        ...
        if filtered_users:
            ...
            user_options = {
                f"{u.get('first_name', '')} {u.get('last_name', '')} ({u.get('email', '')})": u
                for u in filtered_users
            }
            selected_user_display = st.selectbox(
                "Select a user to view details:", list(user_options.keys())
            )
    ...
    if result["success"]:
        ...
        with col2:
            status_filter = st.selectbox(
                "Filter by status",
                ["All", "draft", "published"],
                key="course_status_filter",
            )
        with col3:
            price_filter = st.selectbox(
                "Filter by price", ["All", "Free", "Paid"], key="course_price_filter"
            )
        ...
        if filtered_courses:
            ...
            course_options = {
                f"{c.get('title', '')} - ${c.get('price', 0):.2f}": c
                for c in filtered_courses
            }
            selected_course_display = st.selectbox(
                "Select a course to view details:", list(course_options.keys())
            )
