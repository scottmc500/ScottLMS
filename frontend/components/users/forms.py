    ...
    with st.form(form_key):
        ...
        if submitted:
            if not all([first_name, last_name, email, username, password, password_confirm]):
                ...
                is_valid, errors = validate_password(password, username, first_name, last_name, email)
                if not is_valid:
...
def edit_user_form(user):
    ...
    col1, col2 = st.columns(2)
    with col1:
        first_name = st.text_input("First Name", value=user.get("first_name", ""), key=f"edit_user_first_name_{user.get('id')}")
    with col2:
        last_name = st.text_input("Last Name", value=user.get("last_name", ""), key=f"edit_user_last_name_{user.get('id')}")
    
    email = st.text_input("Email", value=user.get("email", ""), key=f"edit_user_email_{user.get('id')}")
    username = st.text_input("Username", value=user.get("username", ""), key=f"edit_user_username_{user.get('id')}")
    ...
    role = st.selectbox(
        ...
        index=["student", "instructor", "admin"].index(
    ...
    col1, col2 = st.columns(2)
    with col1:
        submitted = st.button("Update User", type="primary", key=f"update_user_btn_{user.get('id')}")
    with col2:
