...
# ACM Certificate (commented out - domain validation issues)
...
resource "aws_route53_record" "scottlms_api" {
  zone_id = aws_route53_zone.scottlms.zone_id
  name    = "api.${var.domain_name}"
  type    = "A"

  alias {
    name                   = aws_lb.scottlms.dns_name
    zone_id                = aws_lb.scottlms.zone_id
    evaluate_target_health = true
  }
}

resource "aws_route53_record" "scottlms_frontend" {
  zone_id = aws_route53_zone.scottlms.zone_id
  name    = var.domain_name
  type    = "A"
