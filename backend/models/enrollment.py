...
class EnrollmentBase(BaseModel):
    ...
    progress_percentage: float = Field(
        default=0.0, ge=0, le=100, description="Course progress percentage"
    )
...
class EnrollmentInDB(EnrollmentBase):
    ...
    enrollment_date: datetime = Field(
        default_factory=datetime.utcnow, description="Enrollment date"
    )
    completion_date: Optional[datetime] = Field(
        None, description="Course completion date"
    )
    last_accessed: Optional[datetime] = Field(
        None, description="Last time student accessed the course"
    )
