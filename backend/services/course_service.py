...
class CourseService:
    ...
    @staticmethod
    async def get_course_with_instructor(
        course_id: PydanticObjectId,
    ) -> Optional[CourseWithInstructor]:
    ...
    async def get_courses(
        ...
        if search:
            ...
            courses = (
                await Course.find({"$text": {"$search": search}}, query)
                .skip(skip)
                .limit(limit)
                .to_list()
            )
    ...
    async def update_course(
        course_id: PydanticObjectId, course_data: CourseUpdate
